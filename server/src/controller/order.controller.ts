...
import { NewOrder } from "../types/User.js";
import Order from "../models/Order.js";
import ErrorResponse from "../utils/ErrorResponse.js";
import { nodeCashe } from "../index.js";


export const createOrder = TryCatch(async (req: Request<{}, {}, NewOrder>, res, next) => {
...
export const myOrders = TryCatch(async (req, res, next) => {
    ...
    if (nodeCashe.has("my-orders")) orders = JSON.parse(nodeCashe.get("my-orders") as string)
    else {
...
export const getSingleOrder = TryCatch(async (req, res, next) => {
    ...
    if (nodeCashe.has(`single-order-${id}`)) {
        order = JSON.parse(nodeCashe.get(`single-order-${id}`) as string)
    } else {
...
export const allOrdersForAdmin = TryCatch(async (req, res, next) => {
    ...
    if (nodeCashe.has("admin-orders")) {
        orders = JSON.parse(nodeCashe.get("admin-orders") as string)
    } else {
