...
export const getDashbordStats = TryCatch(async (req, res, next) => {
    let stats = {};
    if (nodeCashe.has("admin-stats"))
        stats = JSON.parse(nodeCashe.get("admin-stats"));
    else {
        ...
        const [thisMonthProducts, thisMonthUser, thisMonthOrder, lastMonthProducts, lastMonthUser, lastMonthOrder, userCounts, productCounts, allOrders, lastSixMonthsOrder, categories, femaleCounts, latestTransaction] = await Promise.all([
        ...
        const percents = {
            ...
            users: calculatePercentage(thisMonthUser.length, lastMonthUser.length),
            products: calculatePercentage(thisMonthProducts.length, lastMonthProducts.length),
            orders: calculatePercentage(thisMonthOrder.length, lastMonthOrder.length)
        };
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    let charts = {};
    if (nodeCashe.has("admin-pie-charts"))
        charts = JSON.parse(nodeCashe.get("admin-pie-charts"));
    else {
        const allOrderPromise = Order.find({}).select(["total", "disscount", "subTotal", "tax", "shippingCharge"]);
        const [proccesingOrder, shippedOrder, deliveredOrder, categories, productCounts, outOfStock, allOrders, allUsers, adminUser, customUser] = await Promise.all([
