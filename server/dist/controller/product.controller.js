...
export const getLatestProduct = TryCatch(async (req, res, next) => {
    let products;
    if (nodeCashe.has("latestProducts"))
        products = JSON.parse(nodeCashe.get("latestProducts"));
    else {
...
export const adminAllProduct = TryCatch(async (req, res, next) => {
    let products;
    if (nodeCashe.has("admin-products"))
        products = JSON.parse(nodeCashe.get("admin-prodiucts"));
    else {
...
export const getCategory = TryCatch(async (req, res, next) => {
    let productCategory;
    if (nodeCashe.has("catrgories"))
        productCategory = JSON.parse(nodeCashe.get("category"));
    else {
...
export const getProduct = TryCatch(async (req, res, next) => {
    const { id } = req.params;
    let product;
    if (nodeCashe.has(`product${id}`))
        product = JSON.parse(nodeCashe.get(`product${id}`));
    else {
